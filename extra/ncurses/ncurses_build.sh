...
PATH=$tools_prefix/bin:$PATH CFLAGS="$TARGET_CFLAGS" CXXFLAGS="$TARGET_CXXFLAGS" CPPFLAGS="$TARGET_CPPFLAGS" ./configure --host=$AOSC_EC_TRIPLET --prefix=/usr --mandir=/usr/share/man --with-shared --without-debug --enable-pc-files $([ "$CONFIG_EXTRA_NCURSES_WIDE" = "y" ] && echo --enable-widec ) $([ "$CONFIG_EXTRA_NCURSES_XX" != "y" ] && echo --without-cxx-binding ) --includedir=$sysroot/usr/include &&
